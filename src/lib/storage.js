...
export function loadState() {
  try {
    ...
    return normalizeState(parsed);  } catch {
...
export function normalizeState(input) {
  ...
  base.timers = timers.map((t, idx) => ({ ...createTimer(`t${idx + 1}`), ...t, id: t.id ?? `t${idx + 1}` }));  base.stopwatches = stopwatches.map((s, idx) => ({ ...createStopwatch(`s${idx + 1}`), ...s, id: s.id ?? `s${idx + 1}` }));
